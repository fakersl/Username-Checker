...
from config.paths import DATABASE_FILE, ensure_runtime_layout
...
class Database:
    ...
    def save_check_result(self, username: str, platforms_checked: List[str], platforms_available: List[str], session_id: Optional[str] = None):
        with self.lock:
            conn = self.connect()
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def update_session_stats(self, session_id: str, total_checked: int, total_available: int):
        try:
            ...
            self.logger.exception("update_session_stats_failed")
            self.conn = None
